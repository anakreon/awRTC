...
export class Rtc {
    private configuration: RTCConfiguration = null;
    private localMediaStream: MediaStream;
    private peerConnections: { [peerId: string]: PeerConnection }
    ...
    public async initialize (): Promise<void> {
        this.log('rtc initializing');
        
        this.localMediaStream = await this.getUserMediaStream();
    ...
    private assignMediaStreamToPageElements (): void {
        const localAudioElement = <HTMLAudioElement>document.querySelector('audio.id-' + this.currentUserId);
        const localVideoElement = <HTMLVideoElement>document.querySelector('video.id-' + this.currentUserId);
    ...
    private connectToPeers (peerList: string[]): void {
        peerList.forEach(async (peerId: string) => {
            ...
            this.connectToPeerAndSendOffer(peerId);
        });
    }
